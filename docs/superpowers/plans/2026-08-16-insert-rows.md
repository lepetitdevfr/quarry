...
- Add: **a stored generated column is still offered as editable.** `decide_editability` marks any resolved non-key column `editable: true`, so a `GENERATED ALWAYS AS (…) STORED` column opens an editor and then fails at the server with `column "shout" can only be updated to DEFAULT`. Pre-existing — the metadata to know better did not exist until Task 1 of this stage — and now a one-line fix: reuse the `is_generated` helper in the per-column edit verdict, with reason "generated by the database". Left out of this stage to keep the insert diff to inserting.
